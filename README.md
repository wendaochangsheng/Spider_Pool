...
一个面向多域内容编排的内部面板，具备自动伪原创生成、动态互链、外部链接插槽、域名池管理及后台控制台。所有页面默认通过 `robots.txt` 禁止搜索引擎收录，便于在内网或受控环境中搭建内容观测站。
...
4. **后台入口**：访问 `http://localhost:8000/admin`，使用 `admin/admin` 登录即可。
...
│   ├── links.py          # 内外链编排逻辑
│   └── storage.py        # JSON 配置读写
├── templates/            # 前台与后台页面
├── static/css/           # 样式
├── data/site_data.json   # 运行期数据（首次启动会自动创建）
└── requirements.txt
```
...
- 批量生成：一键创建多篇伪原创页面（数量可配置）。
- 自定义页面：指定 slug、话题与关键词，立即生成。
- 域名池：维护泛解析域，方便不同 host 共享池子。
- 外链曝光：添加 / 删除需要优先曝光的外部链接。
- 全局设置：默认关键词、DeepSeek 模型、批量数量等。
- 页面操作：单页查看、再生成、删除。
...
- 所有数据存储在 `data/site_data.json`，可自行定期备份或扩展为数据库。
