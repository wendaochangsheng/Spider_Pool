...
一个面向多域内容编排的内部面板，具备自动伪原创生成、动态互链、外部链接插槽、域名池管理及后台控制台。存储已经切换为 SQLite，支持并发访问、访问量防丢失合并以及爬虫识别。
...
- 🎲 随机生成：一键批量生成随机主题的页面，自动带入默认关键词，保持字数范围。
- 🔎 调试可控：可在配置中心开关 AI 终端日志，避免噪声输出。
- 🤖 robots.txt：统一允许 `User-agent: *` 访问，方便搜索引擎抓取站点内容。
...
- 内容生成：多线程批量生成（默认 8 线程，可配置）、单页生成及再生成、字数范围控制、参考 URL 注入、随机主题批量生成。
- 配置中心：域名池、外链池、关键词与模型设置、AI 终端日志开关，实时同步页面互链。
